...
class JuliusVocabulary(AbstractVocabulary):
    ...
    def _compile_vocabulary(self, phrases):
        ...
        lexicon_file = nikitapath.data('julius-stt', 'VoxForge.tgz')
...
def get_keyword_phrases():
    """
    Gets the keyword phrases from the keywords file in the nikita data dir.
    ...
    with open(nikitapath.data('keyword_phrases'), mode="r") as f:
        for line in f:
            phrase = line.strip()
            if phrase:
