...
Description:    responds to common ways of asking "How are you?"
                determines mood based on weather, stock market,
                and wording of top news stories
...
def getWeather(db, profile):
    ...
    """
    OKConditions = ['thunderstorm', 'chance of a thunderstorm',
                    'rain', 'snow']
    GoodConditions = ['partly cloudy', 'overcast', 'chance of rain',
                      'chance of a thunderstorm', 'cloudy']
...
def getStock():
    """
    Gets stock market condition and returns
    ratings on a 1-5 scale (1 being the best)
    """
    f = urllib2.urlopen('http://finance.google.com/finance/' +
                        'info?client=ig&q=NASDAQ:.IXIC')
    content = f.read()
...
def getNews(db, profile):
    ...
    ratings on a 1-5 scale (1 being the best)
    """
    badWords = ['bomb', 'destruction', 'death', 'dead', 'war', 'hack',
                'casualty', 'woe', 'kill', 'sabotage', 'strike', 'erupt',
                'deception', 'abuse', 'abusive', 'ill', 'disease']
    goodWords = ['peace', 'prosper', 'adorable', 'perfect', 'cheer',
                 'improves', 'happy', 'glad', 'smile', 'joy', 'cure',
                 'heal', 'best', 'win']
    badCount = 0
    goodCount = 2
